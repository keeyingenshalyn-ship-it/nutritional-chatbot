...
def _exclude_keywords(df: pd.DataFrame, keywords: list[str]) -> pd.DataFrame:
    """Remove rows whose Category or Food matches any keyword (single regex pass)."""
    pattern = "|".join(keywords)
    mask = (
        df["_cat"].str.contains(pattern, na=False) |
        df["_food"].str.contains(pattern, na=False)
    )
    return df[~mask]
...
def food_context_str(
    ...
    # Collect all keywords to exclude in one pass
    exclude: list[str] = []
    for a in allergens:
        exclude.extend(ALLERGEN_KEYWORDS.get(a, [a.upper()]))

    if "Vegan" in preferences:
        exclude.extend(ANIMAL_KEYWORDS + ["CHEESE", "BUTTER", "MILK", "CREAM", "YOGURT", "EGG"])
    elif "Vegetarian" in preferences:
        exclude.extend(ANIMAL_KEYWORDS)

    if exclude:
        df = _exclude_keywords(df, exclude)
...
{premium_sections}4. **Health-Specific Notes** — Any important advice relating to the listed
...
def render_step4(api_key: str) -> None:
    ...
    with st.container(border=True):
        st.markdown("#### Plan Access")
        left, right = st.columns([3, 2])

        with left:
            if premium:
                st.success("⭐ **Premium active** — full plan with recipes, grocery list & meal prep tips.")
                include_recipes = True

            elif not trial_used:
                st.success(
                    "🎁 **Free trial available!**  \n"
                    "Unlock recipes, grocery list & meal prep tips **once** at no cost."
                )
                include_recipes = st.checkbox(
                    "Include recipes, grocery list & meal prep tips *(uses your free trial)*",
                    value=False,
                )
                ...
                st.warning(
                    ...
                    "Upgrade to Premium to get recipes, grocery list & meal prep tips on every plan."
                )
                include_recipes = False

        with right:
            st.markdown("**Always included (free):**")
    ...
    if st.session_state.meal_plan is None:
        if st.button("🍽️  Generate My Meal Plan", type="primary", use_container_width=True):
            if not api_key:
                st.error("⚠️ OpenAI API key not configured. Add OPENAI_API_KEY to your Streamlit secrets.")
                return
