...
# Representative foods queried from Edamam per diet type
_OMNIVORE_FOODS = (
    "chicken breast", "salmon", "eggs", "beef sirloin", "tuna",
    "brown rice", "quinoa", "oats", "whole wheat bread", "lentils",
    "black beans", "chickpeas", "broccoli", "spinach", "sweet potato",
    "avocado", "almonds", "Greek yogurt", "cottage cheese", "apple",
)
_VEGAN_FOODS = (
    "tofu", "tempeh", "lentils", "black beans", "chickpeas",
    "quinoa", "oats", "brown rice", "broccoli", "spinach",
    "sweet potato", "avocado", "almonds", "banana", "olive oil",
    "walnuts", "chia seeds", "hemp seeds", "apple", "blueberries",
)
_VEGETARIAN_FOODS = (
    "eggs", "Greek yogurt", "cottage cheese", "tofu", "lentils",
    "chickpeas", "quinoa", "brown rice", "broccoli", "spinach",
    "sweet potato", "almonds", "avocado", "banana", "whole wheat bread",
    "black beans", "feta cheese", "walnuts", "apple", "oats",
)
...
# ─────────────────────────────────────────────────────────────────────────────
# Edamam Food Database helpers
# ─────────────────────────────────────────────────────────────────────────────

def _fetch_one(query: str, app_id: str, app_key: str) -> dict | None:
    """Fetch nutrients for one food from Edamam (per 100 g)."""
    try:
        r = requests.get(
            EDAMAM_FOOD_URL,
            params={
        ...
        return {
            "Food":      food.get("label", query),
            "Category":  food.get("category", "Generic foods"),
            "Calories":  round(n.get("ENERC_KCAL", 0), 1),
            "Protein_g": round(n.get("PROCNT",     0), 1),
            "Carbs_g":   round(n.get("CHOCDF",     0), 1),
            "Fat_g":     round(n.get("FAT",         0), 1),
            "Fiber_g":   round(n.get("FIBTG",       0), 1),
        }
    except Exception:
        return None
...
def food_context_str(
    ...
    """Fetch Edamam nutrition data in parallel and return a CSV for the prompt."""
    try:
        app_id  = st.secrets.get("EDAMAM_APP_ID",  "")
        app_key = st.secrets.get("EDAMAM_APP_KEY", "")
    except Exception:
        return ""
    ...
    # Pick query list based on dietary style
    if "Vegan" in preferences:
        queries = list(_VEGAN_FOODS)
    elif "Vegetarian" in preferences:
        queries = list(_VEGETARIAN_FOODS)
    else:
        queries = list(_OMNIVORE_FOODS)

    # Drop foods that overlap with selected allergens
    exclude_kw: list[str] = []
    for a in allergens:
        exclude_kw.extend(ALLERGEN_KEYWORDS.get(a, [a.upper()]))
    queries = [q for q in queries if not any(kw in q.upper() for kw in exclude_kw)]

    # Fetch all foods in parallel (up to 5 concurrent requests)
    rows: list[dict] = []
    with ThreadPoolExecutor(max_workers=5) as ex:
        futures = {ex.submit(_fetch_one, q, app_id, app_key): q for q in queries}
