...
TRIAL_FILE        = Path(os.path.expanduser("~/.nutribot_trial.json"))
...
def load_food_db() -> pd.DataFrame:
    """Load food.csv and keep only the nutritionally relevant columns."""
    ...
    rename = {
        ...
        "Data.Cholesterol":           "Cholesterol_mg",
        "Data.Major Minerals.Calcium":   "Calcium_mg",
        "Data.Major Minerals.Iron":      "Iron_mg",
        "Data.Major Minerals.Potassium": "Potassium_mg",
        "Data.Major Minerals.Sodium":    "Sodium_mg",
        "Data.Vitamins.Vitamin C":       "VitC_mg",
        "Data.Vitamins.Vitamin A - IU":  "VitA_IU",
    }
    df = df.rename(columns=rename)
    keep = [c for c in rename.values() if c in df.columns]
    return df[keep].dropna(subset=["Calories"])
...
def filter_food_db(
    ...
    """Remove allergens and apply dietary-style filters."""
    if df.empty:
        return df

    mask = pd.Series(True, index=df.index)
    cat  = df["Category"].str.upper()
    food = df["Food"].str.upper()

    # Drop allergen-containing foods
    for allergen in allergens:
        for kw in ALLERGEN_KEYWORDS.get(allergen, [allergen.upper()]):
            mask &= ~(cat.str.contains(kw, na=False) | food.str.contains(kw, na=False))
    ...
    # Vegan: also remove meat + dairy/egg keywords
    if "Vegan" in preferences:
        vegan_exclude = ANIMAL_KEYWORDS + ["CHEESE", "BUTTER", "MILK", "CREAM", "YOGURT", "EGG"]
        vmask = pd.Series(True, index=df.index)
        for kw in vegan_exclude:
            vmask &= ~(cat.str.contains(kw, na=False) | food.str.contains(kw, na=False))
        df = df[vmask]

    # Vegetarian: remove meat only
...
def food_context_str(allergens: list[str], preferences: list[str]) -> str:
    """Return a compact CSV snippet of safe foods to embed in the prompt."""
    df = load_food_db()
    if df.empty:
        return ""

    df = filter_food_db(df, allergens, preferences)
    if df.empty:
        return ""

    # Sample up to 4 foods per category, cap at 120 rows total
    sample = (
...
def trial_is_used() -> bool:
    """Return True if the one-time free recipe trial has been consumed."""
    try:
        if TRIAL_FILE.exists():
            return json.loads(TRIAL_FILE.read_text()).get("used", False)
    except Exception:
        pass
    return False
...
def consume_trial() -> None:
    """Mark the free trial as used (persists across app restarts)."""
    TRIAL_FILE.write_text(json.dumps({"used": True}))
...
def build_prompt(data: dict, include_recipes: bool) -> str:
    ...
    food_ctx = food_context_str(data.get("allergens", []), data.get("preferences", []))
    food_section = (
        f"\n═══════════════════════════════════════\n"
...
def render_step2() -> None:
    ...
    for (threshold, lbl, c), col in zip(BMI_SCALE, scale_cols):
